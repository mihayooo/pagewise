...
describe('R280: ChangelogV340Fix', () => {
  ...
  describe('AC-1: CHANGELOG.md 包含 [3.4.0] 区段', () => {
    ...
    it('CHANGELOG.md 应包含 ## [3.5.0] 区段标题', () => {
      const content = readFile('docs/CHANGELOG.md');
      assert.ok(content.includes('## [3.5.0]'),
        'Should contain ## [3.5.0] header');
    });

    it('[3.5.0] 区段应包含日期 2026-05-26', () => {
      const content = readFile('docs/CHANGELOG.md');
      assert.ok(content.includes('[3.5.0] - 2026-05-26'),
        'Should have [3.5.0] - 2026-05-26');
    });
  });
  ...
  describe('AC-2: [3.4.0] 区段包含 R275-R279 变更', () => {
    ...
    const idx350 = changelogContent.indexOf('## [3.5.0]');
    const idx343 = changelogContent.indexOf('## [3.4.3]');
    const section350 = (idx350 >= 0 && idx343 >= 0)
      ? changelogContent.slice(idx350, idx343)
      : '';

    it('应包含 R278 跨浏览器兼容层', () => {
      assert.ok(section350.includes('R333') || section350.includes('KnowledgeGraphInteraction'),
        'Should reference R333 / KnowledgeGraphInteraction');
    });

    it('应包含 R277 运行时性能优化', () => {
      assert.ok(section350.includes('R332') || section350.includes('SidebarPerfOpt'),
        'Should reference R332 / SidebarPerfOpt');
    });

    it('应包含 R275 WCAG 无障碍合规', () => {
      assert.ok(section350.includes('R331') || section350.includes('SearchHistoryPersist'),
        'Should reference R331 / SearchHistoryPersist');
    });
  ...
  describe('AC-3: 版本号一致性 3.5.0', () => {
    ...
    it('CHANGELOG 应包含 [3.5.0]（修复 test-r197:174 失败）', () => {
      const changelog = readFile('docs/CHANGELOG.md');
      assert.ok(changelog.includes('[3.5.0]'),
        'CHANGELOG must contain [3.5.0] — fixes test-r197-version-sync.js:174');
    });
  });

  // ── AC-4: 版本区段降序排列 ──
  describe('AC-4: CHANGELOG 版本区段降序排列', () => {
    it('[3.5.0] 应位于 [3.4.3] 之前', () => {
      const content = readFile('docs/CHANGELOG.md');
      const idx350 = content.indexOf('## [3.5.0]');
      const idx343 = content.indexOf('## [3.4.3]');
      assert.ok(idx350 >= 0, '[3.5.0] should exist');
      assert.ok(idx343 >= 0, '[3.4.3] should exist');
      assert.ok(idx350 < idx343, '[3.5.0] should appear before [3.4.3]');
    });
  ...
  describe('AC-5: 测试文件中无硬编码 3.3.0 断言', () => {
    ...
    it('test-r197 应断言 changelog 包含 [3.5.0]', () => {
      const content = readFile('tests/test-r197-version-sync.js');
      assert.ok(content.includes("changelog.includes('[3.5.0]')"),
        'Should check for [3.5.0] in changelog');
    });

...
// ============================================================
// 测试合并自 test-bookmark-backup.js (R115 TestSuiteTrim)
// ============================================================
...
// R115 merge: removed duplicate import statements, using top-level declarations

const {
  SUPPORTED_VERSIONS,
  createBackup: _createBackup,
  validateBackup: _validateBackup,
  restoreBackup,
...
describe('createBackup — 正常创建', () => {
  it('成功创建备份，包含所有必需字段', () => {
    const bookmarks = [bm(1, 'Test', 'https://test.com')]
    const result = _createBackup(bookmarks)
  ...
  it('包含 description 选项', () => {
    const bookmarks = [bm(1, 'Test', 'https://test.com')]
    const result = _createBackup(bookmarks, { description: '手动备份' })
...
describe('createBackup — 大数据集', () => {
  ...
  it('大数据集的校验和可验证', () => {
    ...
    const validation = _validateBackup(result.backup)
    assert.equal(validation.valid, true)
  })
})
...
describe('createBackup — 深拷贝隔离', () => {
  ...
  it('不同调用返回不同的 backupId', () => {
    const b1 = _createBackup([])
    const b2 = _createBackup([])
    assert.notEqual(b1.backup.backupId, b2.backup.backupId)
  })
})
...
describe('createBackup — 无效输入', () => {
  it('非数组输入返回失败', () => {
    const result = _createBackup('not-an-array')
    assert.equal(result.success, false)
    assert.ok(result.errors.length > 0)
  })

  it('null 输入返回失败', () => {
    const result = _createBackup(null)
    assert.equal(result.success, false)
  })

  it('undefined 输入返回失败', () => {
    const result = _createBackup(undefined)
    assert.equal(result.success, false)
  })
})
...
describe('validateBackup — 有效备份', () => {
  it('刚创建的备份验证通过', () => {
    const bookmarks = [bm(1, 'Test', 'https://test.com')]
    const { backup } = _createBackup(bookmarks)

    const result = _validateBackup(backup)
    assert.equal(result.valid, true)
    assert.equal(result.errors.length, 0)
  })
...
describe('validateBackup — null / undefined / 非对象', () => {
  it('null 输入返回无效', () => {
    const result = _validateBackup(null)
    assert.equal(result.valid, false)
    assert.ok(result.errors.length > 0)
  })

  it('undefined 输入返回无效', () => {
    const result = _validateBackup(undefined)
    assert.equal(result.valid, false)
    assert.ok(result.errors.length > 0)
  })

  it('字符串输入返回无效', () => {
    const result = _validateBackup('not-an-object')
    assert.equal(result.valid, false)
  })

  it('数组输入返回无效', () => {
    const result = _validateBackup([1, 2, 3])
    assert.equal(result.valid, false)
  })

  it('数字输入返回无效', () => {
    const result = _validateBackup(42)
    assert.equal(result.valid, false)
  })
})
...
describe('validateBackup — 损坏的备份', () => {
  it('篡改书签数据后校验和不匹配', () => {
    const { backup } = _createBackup([bm(1, 'Test', 'https://test.com')])
...
describe('restoreBackup — 成功恢复', () => {
  it('从有效备份恢复书签', () => {
    const bookmarks = [bm(1, 'A', 'https://a.com'), bm(2, 'B', 'https://b.com')]
    const { backup } = _createBackup(bookmarks, { metadata: { source: 'test' } })
  ...
  it('恢复时包含 metadata', () => {
    const meta = { description: '备份', version: 2 }
    const { backup } = _createBackup([bm(1, 'X', 'https://x.com')], { metadata: meta })
...
describe('restoreBackup — 深拷贝隔离', () => {
  ...
  it('修改恢复数据不影响原始备份', () => {
    const { backup } = _createBackup([bm(1, 'X', 'https://x.com')], { metadata: { tag: 'original' } })
...
describe('restoreBackup — 部分恢复', () => {
  it('跳过结构无效的书签', () => {
    const { backup } = _createBackup([bm(1, 'Valid', 'https://valid.com')])
    // 注入无效书签
...
describe('完整 round-trip: 创建 → 验证 → 恢复', () => {
  it('原始书签数据与恢复后数据完全一致', () => {
    ...
    // 1. 创建备份
    const { success, backup } = _createBackup(original, options)
    assert.equal(success, true)
  ...
  it('大数据集 round-trip 保持完整性', () => {
    ...
    const { success, backup } = _createBackup(original)
    assert.equal(success, true)

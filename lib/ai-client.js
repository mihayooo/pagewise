...
import {
  ...
  explainTermFn,
} from './ai-client-context-methods.js'
...
{"summary": "...", "tags": ["tag1", "tag2", "tag3"]}`
  ...
  // ==================== 上下文感知增强 (R164, 委托至 ai-client-context-methods) ====================

  async askAboutPageWithContext(pageContent, question, options = {}) {
    return askAboutPageWithContextFn(pageContent, question, {
      ...options,
  ...
  async *askAboutPageWithContextStream(pageContent, question, options = {}) {
    yield* askAboutPageWithContextStreamFn(pageContent, question, {
      ...options,
      chatStream: (msgs, opts) => this.chatStream(msgs, opts),
    })
  }

  async explainTerm(term, pageContent, options = {}) {
    return explainTermFn(term, pageContent, {
      ...options,
      chat: (msgs, opts) => this.chat(msgs, opts),
      model: this.model,
    })

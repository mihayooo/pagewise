...
export class MemorySystem {
  ...
  async loadUserProfile() {
    try {
      ...
      console.warn('[memory] Failed to load user profile, using defaults:', err.message);
      this.userProfile = {
        level: 'intermediate',
        languages: [],
        domains: [],
  ...
  async recall(query, aiClient = null) {
    ...
    if (aiClient && topEntries.length > 3) {
      try {
        ...
        console.warn('[memory] AI rerank failed, using score-based ranking:', err.message);
      }
    }
...
只返回最相关的条目编号，用逗号分隔，如：0,3,1,5,2`
    ...
    try {
      ...
      console.warn('[memory] Failed to parse AI rerank response, using top entries:', err.message);
  ...
  async autoSaveIfWorth(question, answer, pageContext, aiClient) {
    ...
    try {
      ...
      console.warn('[memory] Failed to auto-save entry:', err.message);
      return null;
    }
  }
  ...
  extractDomain(url) {
    ...
    } catch (e) { console.warn('[Memory] save error:', e?.message || e);
      // Invalid URL is expected input — silent null return is correct
      return null;
    }
  }

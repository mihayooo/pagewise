...
export function createBackup(bookmarks, metadata = {}) {
  ...
  try {
    ...
    console.warn('[PageWise]', 'bookmark-backup-restore: createBackup serialize failure', e)
  ...
  if (metadata && typeof metadata === 'object') {
    try {
      ...
      console.warn('[BackupRestore]', e?.message || e);
      /* safe: metadata is optional, default to empty object on failure */
      metadataCopy = {}
    }
  }
...
export function restoreFromBackup(backup) {
  ...
  try {
    ...
    console.warn('[PageWise]', 'bookmark-backup-restore: restoreFromBackup deserialize failure', e)
    return {
      success: false,
      bookmarks: null,
      metadata: null,
  ...
  if (backup.metadata && typeof backup.metadata === 'object') {
    try {
      ...
      console.warn('[BackupRestore]', e?.message || e);
      /* safe: metadata is optional, already warns via warnings array */

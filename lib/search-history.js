/**
 * SearchHistory — 搜索历史持久化模块
 *
 * 基于 IndexedDB 持久化搜索历史记录：
 *   - recordSearch / getSearchHistory / getSearchSuggestions
 *   - exportSearchHistory / clearSearchHistory
 *   - isHistoryEnabled / setHistoryEnabled
 *   (AI 高亮功能已拆分至 ai-highlight-store.js)
 *
 * 约束: 纯 ES Module，最多 200 条，按 LRU 淘汰，归一化去重
 */
...
export const HIGHLIGHT_STORE = 'aiHighlights'
...
async function _getEnabledFromStorage() {
  try {
    ...
    console.warn('[SearchHistory]', e?.message || e);
    return true
  }
}

/** 搜索历史是否启用 @returns {Promise<boolean>} */
...
/** 设置搜索历史开关 @param {boolean} enabled */
    ...
    getAllReq.onsuccess = () => {
      ...
      addReq.onsuccess = () => {
        ...
        if (all.length >= MAX_HISTORY) {
          ...
          let done = 0
          const finish = () => { if (++done >= toDelete.length) resolve(record) }
          for (const item of toDelete) {
            store.delete(item.id).onsuccess = finish
            store.delete(item.id).onerror = finish
          }
        } else {
          resolve(record)
        }
...
/**
 * 搜索建议：前 2 字符匹配历史记录 Top-N
 * 权重: score = count * 0.6 + recencyScore * 0.4
 */
...
// ==================== AI 问答高亮（已拆分至 ai-highlight-store.js） ====================
export {
  saveAIHighlight,

...
class SelectionHandler {
  ...
  _guessLanguage(code) {
    ...
    if (/\b(fmt\.Print\b|func\s+\w+\s*\(|package\s+\w+\b)/.test(code)) return 'go';
  ...
  _extractDomain(url) {
    try { return new URL(url).hostname; } catch (e) { console.warn('[SelectionHandler] URL parse failed:', e?.message); return ''; }
  }
  ...
  _safeEval(expr) {
    ...
    try {
      ...
      return 'N/A';
    } catch (e) { console.warn('[SelectionHandler]', e?.message || e); return 'N/A'; }
  }

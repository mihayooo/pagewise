...
async function _doDetect() {
  ...
  try {
    const ok = await new Promise((resolve) => {
      try {
        chrome.storage.sync.get({ __pw_sync_test: null }, (_result) => {
        ...
        /* safe: sync feature detection probe — throw means sync unavailable */
        resolve(false)
      }
    })
    ...
    /* safe: storage detection failure expected in non-extension environments */
    _backend = 'local'
    return false
  }
}
...
export async function storageGet(keys) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      console.warn('[PageWise]', 'storageGet error', _e)
      resolve(typeof keys === 'object' && !Array.isArray(keys) ? { ...keys } : {})
    }
  })
}
...
export async function storageSet(items) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      console.warn('[PageWise]', 'storageSet error', _e)
      resolve()
    }
  })
}
...
export async function storageRemove(keys) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      console.warn('[PageWise]', 'storageRemove error', _e)
      resolve()
    }
  })
}

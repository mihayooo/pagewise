...
export async function getStats() {
  try {
    ...
    console.warn('[PageWise]', 'getStats error', _e)
...
async function saveStats(stats) {
  ...
  try { await chrome.storage.local.set({ [STATS_KEY]: stats }); } catch (e) { console.warn('[PageWise]', 'saveStats error', _e) }
    console.debug("[PageWise] stats:saveStats failed", e);
}

/**
 * @param {string} key - 统计键名
 * * @param {number} [value=1] - 递增值
 * * @returns {Promise<number>} 更新后的值
 */
 ...
 * * @param {object} data - 使用数据 { questions, tokens, highlights }
 */
...
/**
 * @param {object} dailyUsage - 每日使用数据
 * @returns {number} 连续使用天数
 */
 ...
 * @param {number} [limit=5] - 返回数量上限
 * @returns {Array<{tag: string, count: number}>} 标签使用排行
 */
 ...
 * @returns {Array<{word: string, count: number}>} 高频词列表
 */
...
/**
 * @param {Array} entries - 知识条目列表
 * @param {number} [weeks=8] - 回溯周数
 * @returns {Array<{weekLabel: string, count: number}>} 每周增长趋势
 */
...
/** @param {object} budget - 预算配置 { dailyCents, monthlyCents } */
export async function setBudget(budget) {
  return _costOps.setBudget(budget);
}
/** @returns {Promise<object>} 费用摘要 */
export async function getCostSummary() {
  return _costOps.getCostSummary();
}
/** @param {number} [days=30] - 回溯天数 @returns {Promise<Array>} 费用趋势 */
...
export function _createStatsModule(storageImpl) {
  ...
  async function getStatsInner() {
    try {
      ...
      const stored = result[STATS_KEY] || {};
      return { ...DEFAULT_STATS, ...stored, skillUsage: { ...DEFAULT_STATS.skillUsage, ...stored.skillUsage }, modelUsage: { ...DEFAULT_STATS.modelUsage, ...stored.modelUsage }, dailyUsage: { ...DEFAULT_STATS.dailyUsage, ...stored.dailyUsage } };
    } catch (e) { console.warn('[PageWise]', 'getStatsInner error', _e); return { ...DEFAULT_STATS }; }
      console.debug("[PageWise] stats:getStatsInner failed", e);
  }

  async function saveStatsInner(stats) {
    stats.lastUpdated = Date.now();
    try { await impl.set({ [STATS_KEY]: stats }); } catch (e) { console.warn('[PageWise]', 'saveStatsInner error', _e) }
      console.debug("[PageWise] stats:getStatsInner failed", e);
  }

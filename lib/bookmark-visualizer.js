...
export class BookmarkVisualizer {
  ...
  constructor(canvas) {
    ...
    // R277: 脏区域检测 & 大图谱降级
    /** @type {Set<string>} 本帧发生位移变化的节点 ID */
    this._dirtyNodes = new Set()
    /** @type {boolean} 大图谱降级模式（>500 节点时自动激活） */
    this._degraded = false
    /** @type {number} 降级模式下的帧率限制（每 N tick 渲染一次） */
    this._degradeFrameSkip = 0
    this._tickCount = 0
  ...
  render(graphData) {
    ...
    // R277: 大图谱降级策略（>500 节点）
    this._degraded = graphData.nodes.length > 500
    if (this._degraded) {
      // 每 3 tick 才渲染一次，减少动画帧率
      this._degradeFrameSkip = 2
    } else {
      this._degradeFrameSkip = 0
    }
    this._tickCount = 0
  ...
  _tick() {
    ...
    // R277: 帧率降级 — 大图谱模式下每 N tick 才渲染
    this._tickCount++
    const shouldRender = this._degradeFrameSkip <= 0 ||
      (this._tickCount % (this._degradeFrameSkip + 1) === 0)

...
import { createFloatingPanel, destroyFloatingPanel } from './chat-mode-panel.js';
...
class ChatMode {
  ...
  _createFloatingPanel() {
    ...
    this._floatingEl = createFloatingPanel({
      pageContext: this._pageContext,
      onSwitchMode: () => this.toggleDisplayMode(),
      onClose: () => this.close(),
      onSend: (text) => {
  ...
  _destroyFloatingPanel() {
    destroyFloatingPanel(this._floatingEl);
    this._floatingEl = null;
  }
  ...
  async _saveDisplayMode() {
    ...
    try {
      ...
      console.warn('[PageWise]', 'chat display mode save error', _e)
    }
  }
  ...
  async _restoreDisplayMode() {
    ...
    try {
      ...
      console.warn('[PageWise]', 'chat display mode restore error', _e)
    }
  }
  ...
  _emitOpen() {
    ...
    try {
      ...
      console.debug("[PageWise] chat-mode:message failed", e);
      /* safe: chrome.runtime.sendMessage unavailable (extension context invalidated) */
    }
  }
  ...
  _emitClose() {
    try {
      ...
      console.debug("[PageWise] chat-mode failed", e);
      /* safe: chrome.runtime.sendMessage unavailable (extension context invalidated) */
    }
  }

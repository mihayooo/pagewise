...
export function matchesDomain(bookmark, domain) {
  ...
  } catch (e) { console.warn('[SmartCollections] domain match failed:', e?.message);
    return false;
  }
}
...
export function matchesCategory(bookmark, category) {
  ...
  // 简单关键词匹配
  const urlDomain = (() => {
    try { return new URL(bookmark.url).hostname.toLowerCase(); } catch (e) { console.warn('[SmartCollections] hostname parse failed:', e?.message); return ''; }
  })();

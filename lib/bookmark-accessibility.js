...
export class BookmarkAccessibility {
   ...
   * @param {boolean} [replace=false] — true 时完全替换，false 时追加
   */
  static setContrastPairs(pairs, replace = false) {
    if (replace) {
   ...
   * @returns {Array<{ selector: string, foreground: string, background: string, ratio: number, passes: boolean }>}
   */
  static getFailingPairs() {
    return BookmarkAccessibility.auditContrast().filter(item => !item.passes)
  }

  /**
   * 对比度审计摘要（向后兼容：不修改 auditContrast 返回值）
   * @returns {{ results: Array, total: number, passing: number, failing: number }}
   */
  static auditContrastSummary() {

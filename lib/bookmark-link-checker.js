  ...
  async checkAll(bookmarks) {
    ...
    const worker = async () => {
      while (index < bookmarks.length && !this._cancelled) {
        ...
        if (this.onProgress) {
          try { this.onProgress(this.results.length, total, result); } catch (_e) { console.debug('[PageWise] link-checker: onProgress callback error', _e); }
        }
      }
    };
    ...
    if (this.onComplete) { try { this.onComplete(report); } catch (_e) { console.debug('[PageWise] link-checker: onComplete callback error', _e); } }
    return report;
  }

...
export class BookmarkGraphEngine {
  constructor() {
    ...
    /** @type {Map<string, string[]>} id → cached tokenized title tokens */
  ...
  buildGraph(bookmarks) {
    ...
    for (const bm of bookmarks) {
      ...
      this._tokenCache.set(id, tokens);
  ...
  _extractDomain(url) {
    if (!url) return '';
    try { return new URL(url).hostname.replace(/^www\./, '').toLowerCase(); } catch (e) { console.warn('[GraphEngine] URL parse failed:', url, e?.message); return ''; }
  }

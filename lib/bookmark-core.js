...
/**
 * 书签采集器（re-export 自 bookmark-collector.js）
 */
export class BookmarkCollector {
  ...
  getStats() {
    ...
    for (const bm of this.bookmarks) {
      ...
      } catch (e) { console.warn('[BookmarkCore] domain parse failed:', e?.message); domainDistribution['unknown'] = (domainDistribution['unknown'] || 0) + 1; }
    }
...
/**
 * 书签索引器（re-export 自 bookmark-indexer.js）
 */
...
/**
 * 书签状态管理器（re-export 自 bookmark-status.js）
 */
...
export class BookmarkContentPreview {
  static extractUrlInfo(url) {
    ...
    } catch (e) { console.warn('[BookmarkCore] parseUrl failed:', e?.message); return { domain: '', path: '', protocol: '', favicon: '' }; }
  }
...
function _tokenizeUrl(url) {
  ...
  } catch (e) { console.warn('[BookmarkCore] invalid URL:', e?.message); }
  return tokens;
}

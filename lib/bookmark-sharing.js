...
class BookmarkSharing {
  ...
  static importSharedCollection(data) {
    ...
    if (data.startsWith('data:')) {
      ...
      try {
        ...
        console.warn('[BookmarkSharing]', e?.message || e);
        /* safe: data might not be Base64, fall through to treat as JSON */
        jsonStr = data;
      }
    }
    ...
    try {
      ...
      console.warn('[BookmarkSharing]', e?.message || e);
      /* safe: invalid import data → return null as expected error response */
      return null;
    }
  ...
  static _anonymizeUrl(url) {
    try {
      ...
      console.warn('[PageWise]', 'bookmark-sharing: _anonymizeUrl invalid URL', e)
    ...
    for (const bm of bookmarks) {
      if (bm.url) {
        try {
          ...
          console.warn('[PageWise]', 'bookmark-sharing: _buildMetadata invalid URL', e)
        }
      }

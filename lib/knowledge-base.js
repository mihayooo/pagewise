...
export class KnowledgeBase {
  constructor() {
    ...
    this._queryCache = new Map();       // Map<key, { value, expiresAt }>
    this._queryCacheMaxSize = 50;
    this._queryCacheTTL = 60_000;       // 默认 TTL: 60 秒
  }
   ...
   * @returns {*} - 缓存值，或 undefined（未命中/过期）
   */
  _getQueryCache(key) {
  ...
  /**
   * 向查询缓存写入（带 TTL + LRU 淘汰）
   * @param {string} key - 缓存键
   * @param {*} value - 缓存值
   * @param {number} [ttl] - TTL（毫秒），默认使用 this._queryCacheTTL
   */
  ...
  /**
   * 按分类查询条目（利用 category_createdAt 复合索引，按时间倒序）
   * @param {string} category - 分类名称
   * @param {number} [limit=100] - 返回数量上限
   * @returns {Promise<Array<Object>>} - 匹配的条目
   */
  async getEntriesByCategory(category, limit = 100) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 尝试使用 category_createdAt 复合索引
      let index;
      try {
        index = store.index('category_createdAt');
      } catch (e) {
        // 回退到单字段 category 索引
      ...
      const results = [];
      const isCompound = index.name === 'category_createdAt';

      if (isCompound) {
        // 复合索引: 使用 IDBKeyRange 匹配 category 前缀
        ...
        request.onsuccess = () => {
          ...
          this._setQueryCache(cacheKey, sliced);
          resolve(sliced);
        };
  ...
  /**
   * 按标题前缀查询条目（利用 title_createdAt 复合索引）
  ...
  async getEntriesByTitlePrefix(titlePrefix, limit = 100) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let index;
      try {
        index = store.index('title_createdAt');
      } catch (e) {
        // 回退: 全量扫描标题
        const allReq = store.index('createdAt').openCursor(null, 'prev');
        const results = [];
        const lowerPrefix = titlePrefix.toLowerCase();
        allReq.onsuccess = (event) => {
          ...
          if ((cursor.value.title || '').toLowerCase().startsWith(lowerPrefix)) {
            results.push(cursor.value);
          }
          cursor.continue();
        };
        allReq.onerror = () => reject(new Error('标题前缀查询失败'));
        return;
      }

      // 使用 title_createdAt 复合索引的 IDBKeyRange
      const results = [];
      const range = IDBKeyRange.bound(
        [titlePrefix, ''],
        [titlePrefix + '￿', '￿'],
  ...
  /**
   * 按分类和日期范围查询条目（利用 category_createdAt 复合索引）
   * @param {string} category - 分类名称
   * @param {string} startDate - ISO 起始日期（含）
   * @param {string} endDate - ISO 结束日期（含）
   * @param {number} [limit=100] - 返回数量上限
   * @returns {Promise<Array<Object>>} - 匹配的条目
   */

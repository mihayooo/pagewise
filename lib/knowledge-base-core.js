...
export class KnowledgeBaseCore {
  constructor() {
    ...
    this.dbVersion = 3; // R277: v3 添加 (type+updatedAt) 复合索引
    this.db = null;
    // 性能优化：LRU 搜索缓存（使用 Map 保持与测试兼容的 .size 属性）
    this._searchCache = new Map();
    /** @type {number} 搜索缓存最大条目数 */
    ...
    // 查询结果缓存层（R127 统一缓存层）
    this._queryCache = new CacheManager({ maxSize: 50, ttlMs: 60_000 });
  }
  ...
  async init() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // R277: 升级到 v3 — 添加 (type+updatedAt) 复合索引，优化按类型筛选+时间排序的查询
        if (oldVersion < 3 && db.objectStoreNames.contains('entries')) {
          const tx = event.target.transaction;
          const store = tx.objectStore('entries');
          const existingIndexes = store.indexNames;

          if (!existingIndexes.contains('type_updatedAt')) {
            store.createIndex('type_updatedAt', ['category', 'updatedAt'], { unique: false });
          }
        }
  ...
  _setCachedSearch(key, value) {
    ...
    if (!this._searchCache.has(key)) {
      while (this._searchCache.size >= this._searchCacheMaxSize) {
  ...
  getQueryCacheStats() {
    const s = this._queryCache.stats();
    return {
      size: s.size,
      maxSize: s.maxSize,
      ttl: this._queryCache.ttlMs,
    };
  }
}

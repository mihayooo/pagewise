...
export async function throttleDomain(ctx, url) {
  ...
  } catch (e) { console.warn('[LinkChecker] invalid URL for throttle:', e?.message);
    return; // 无效 URL，不做限流
  }

  const delay = ctx._domainThrottleMs ?? 500;
  const now = Date.now();
...
export function isValidUrl(url) {
  try {
    new URL(url);
    return true;
  } catch (e) { console.warn('[LinkChecker] URL validation failed:', e?.message);
    return false;
  }
}

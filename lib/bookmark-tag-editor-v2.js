...
import { TECH_KEYWORDS, _extractDomainTag, _extractPathTags, _escapeRegex } from './tag-editor-constants.js';
import { getUnusedTags, getTagCooccurrence, prioritizeSuggestionCandidates } from './bookmark-tag-analytics.js';

// ==================== BookmarkTagEditorV2 ====================

/** BookmarkTagEditorV2 类 */
export class BookmarkTagEditorV2 {
  ...
  getTagSuggestions(bookmark, limit = 5) {
    ...
    // 1. 域名标签
    const domainTag = _extractDomainTag(bookmark.url);
    if (domainTag && !existingBmTags.has(domainTag)) {
      candidates.push(domainTag);
    }

    // 2. URL 路径标签
    const pathTags = _extractPathTags(bookmark.url);
    ...
    for (const kw of TECH_KEYWORDS) {
      const regex = new RegExp(`(?:^|[\\s_\\-/\\.])${_escapeRegex(kw)}(?:$|[\\s_\\-/\\.])`, 'i');
    ...
    // 去重 + 优先级排序
    return prioritizeSuggestionCandidates(candidates, this.#existingTags, limit);
  }
   ...
   */
  getUnusedTags() {
    return getUnusedTags(this.#bookmarks, this.#existingTags);
  }
  ...
  getTagCooccurrence(minCount = 2) {
    return getTagCooccurrence(this.#bookmarks, minCount);
  }

...
export class KnowledgeBaseExport extends KnowledgeBaseQuery {
  ...
  async getAggregations() {
    ...
    const _sort = (obj) => Object.entries(obj).map(([k, c]) => ({ [k === 'tag' ? 'tag' : Object.keys({k})[0]]: k, count: c })).sort((a, b) => b.count - a.count);
  ...
  async getEntriesByCategory(category, limit = 100) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let index;
      try { index = store.index('category_createdAt'); } catch (_e) { index = store.index('category'); }
      const results = [];
  ...
  async getEntriesByTitlePrefix(titlePrefix, limit = 100) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const store = tx.objectStore('entries');
      let index;
      try { index = store.index('title_createdAt'); } catch (_e) {
        const results = [];

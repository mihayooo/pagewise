...
- [ ] **R387: bookmark-tag-editor-v2.js 单元测试覆盖 TagEditorV2Test** ⚠️ 误标完成 — test-bookmark-tag-editor-v2.js 从未创建 — `lib/bookmark-tag-editor-v2.js` (345行) 标签编辑器 V2 版本，当前零测试覆盖；(1) 新建 `tests/test-bookmark-tag-editor-v2.js`；(2) 测试标签增删改查；(3) 测试自动补全逻辑；(4) 测试批量操作；(5) 目标 ≥20 用例。验收标准: `npm run test:ci` 0 fail。复杂度: Medium
...
- [ ] **R392: bookmark-documentation-data.js 单元测试覆盖 DocDataTest** ⚠️ 误标完成 — test-bookmark-documentation-data.js 从未创建 — `lib/bookmark-documentation-data.js` (316行) 文档数据管理模块，当前零测试覆盖；(1) 新建 `tests/test-bookmark-documentation-data.js`；(2) 测试数据加载与缓存逻辑；(3) 测试数据查询与过滤；(4) 测试数据更新与持久化；(5) 目标 ≥20 用例。验收标准: `npm run test:ci` 0 fail。复杂度: Medium
...
- [ ] **R403: bookmark-learning-progress-db.js 测试覆盖提升 LearnProgCovBoost** ⚠️ 误标完成 — test-bookmark-learning-progress-db.js 从未创建（注: test-bookmark-learning-progress-db-unit.js 存在但覆盖极低） — `lib/bookmark-learning-progress-db.js` (292行) 当前行覆盖率仅 44.9%；(1) 新建 `tests/test-bookmark-learning-progress-db.js`；(2) 测试数据库初始化与 schema 创建；(3) 测试学习进度 CRUD: add/update/get/delete 操作；(4) 测试查询: 按书签ID/状态/日期范围过滤；(5) 测试边界: null 输入/空数据/重复添加/并发写入；(6) 目标 ≥20 用例，行覆盖率 ≥70%。验收标准: `npm run test:ci` 0 fail。复杂度: Medium
...
- [x] **R422: ai-cache.js + knowledge-graph-utils.js 函数覆盖率修复 AICacheKGraphCovFix** ✅ 2026-06-19 — 排查确认: 两个模块测试文件正确且覆盖率达标（ai-cache.js 100% funcs 14/14, knowledge-graph-utils.js 100% funcs 10/10），CI 中显示 0% 是 c8 + `--test-concurrency=8` 的已知 instrumentation 问题（见 R423），非测试质量问题。验收: `c8 --include 'lib/ai-cache.js' node --test tests/test-ai-cache.js` → Functions 100%，`c8 --include 'lib/knowledge-graph-utils.js' node --test tests/test-knowledge-graph-utils-unit.js` → Functions 90%+

## 自动生成任务 — 2026-06-19 20:04 (c8 覆盖率治理 + 零测试模块)

> 由飞轮迭代引擎生成（基于覆盖率分析：19 个模块 CI 显示 0% 函数覆盖率，其中 15 个已有测试文件但 c8+concurrency 未追踪，4 个真正零测试）

- [ ] **R423: c8 + test-concurrency 覆盖率追踪失效修复 C8ConcurrencyFix** — `npm run test:coverage` 使用 `--test-concurrency=8` 运行测试，导致 c8 的 V8 coverage profiler 无法追踪在子线程中加载的模块函数调用，19 个 lib 模块显示 0% 函数覆盖率（实际均有测试且单独运行时 100%）；根因: Node.js `--test` 的并发模式将测试文件分发到 worker threads，c8 只能追踪主线程的 V8 coverage data；(1) 方案 A: 将 `test:ci:coverage` 的 `--test-concurrency` 从 8 降为 1，确保所有测试在主线程运行，c8 可完整追踪；(2) 方案 B: 使用 Node.js 20+ 的 `--experimental-test-coverage` 原生覆盖率替代 c8（需验证与现有 .c8rc.json 的兼容性）；(3) 方案 C: 在 `package.json` 中新增 `test:coverage:single` 脚本使用 `--test-concurrency=1`，保留现有 `test:ci` 不变；(4) 验证: 修复后 `npm run test:coverage` 显示 ai-cache.js Functions ≥80%、knowledge-graph-utils.js Functions ≥80%、docmind-client.js Functions ≥80%；(5) 记录分析至 `docs/reports/c8-concurrency-coverage-fix.md`。验收标准: `npm run test:coverage` 后 coverage-summary.json 中无模块显示 0% 函数覆盖率（排除确实无测试的模块）。复杂度: Medium

- [ ] **R424: 3 个无测试文件模块补全 UntestedModuleTests** — 3 个 lib 模块完全无测试文件且无对应 TODO 任务；(1) `lib/docmind-client-ai.js` (124行, 4 函数): `getAIConfig(client)`/`syncAIConfig(client, config)`/`getAvailableModels(client)`/`getAIUsage(client)` — 新建 `tests/test-docmind-client-ai.js`，mock client._request/_ensureConnected，测试成功路径+API 失败+未连接+空响应；≥16 用例；(2) `lib/page-sense-html.js` (84行, 4 函数): `extractContent(html)`/`extractImages(html)`/`extractMetadata(html)`/`extractHeadings(html)` — 新建 `tests/test-page-sense-html.js`，测试空 HTML/script 标签去除/特殊字符解码/图片 src 提取/metadata 提取/标题层级提取；≥18 用例；(3) `lib/bookmark-detail-panel-helpers.js` (48行, 3 函数): `emitAction(callbacks, action, data)`/`formatDate(timestamp)`/`formatFolderPath(path)` — 新建 `tests/test-bookmark-detail-panel-helpers.js`，测试回调异常容错/日期格式化/文件夹路径格式化；≥12 用例。验收标准: `npm run test:ci` 0 fail，新测试 ≥46 用例。复杂度: Medium
